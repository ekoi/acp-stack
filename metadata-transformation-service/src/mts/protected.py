...
@router.post(
    "/upload-xsl/{xslt_file_name}/{save}", status_code=201, tags=["Upload XSLT"]
)
async def submit_xsl(
    xslt_file_name: str, submitted_xsl: Request, save: bool | None = False
):
    """
...
@router.post(
    "/upload-xsl/{xslt_name}/{xsl_url:path}/{save}",
    status_code=201,
    tags=["Upload XSLT"],
)
...
async def transform(
    ...
    if xslt_name not in data.keys():
        raise HTTPException(
            status_code=500, detail=f'The given xslt_name: "{xslt_name}" is not found'
        )
...
async def transform_specified_outpu(
    xslt_name: str, output_type: OutputFormat, submitted_json_or_xml: Request
):
    """
    ...
    if output_type == output_type.json:
        try:
            ...
            return HTTPException(
                status_code=500,
                detail=f"Output format isn't valid {output_type} format.",
            )
...
async def transform(
    ...
    try:
        ...
        raise HTTPException(
            status_code=404, detail=f"RequestException from {source_url}"
        )
    ...
    if content_type in [
        ...
        if content_type in [
            ...
            if validate_json(response.content.decode("UTF-8")):
                ...
                str_xml = await validate_xml_encapsulated_json(
                    submitted_json, xml_tmp_file_name
                )
            else:
    ...
    result = await transform_to_string(
        str_xml, submitted_json, xml_tmp_file_name, xslt_name
    )
...
@router.post(
    "/transform-jsonld-to-rdf",
    tags=["Transform"],
    name="Transform json-ld to RDF-XML format.",
...
@router.post(
    "/transform-jsonld-to-rdf/{output_format}",
    tags=["Transform"],
...
@router.post(
    "/transform-jsonld-to-rdf/{output_format}/{source_url:path}",
    tags=["Transform"],
    name="Transform json-ld to a given RDF output format.",
)
...
@router.post(
    "/transform-xml-to-json/{clean_output}",
    tags=["Transform"],
    name="Transform xml to json format.",
...
def delete_saved_xsl(xslt_name: str):
    raise HTTPException(
        status_code=501, detail=f"This endpoint is not implemented yet."
    )


async def create_xml_tmp_file_name(xslt_name):
    xml_tmp_file_name = (
        f"{settings.TEMP_TRANSFORM_FILE}-{str(uuid.uuid1())}-{xslt_name}.xml"
    )
...
def remove_xml_invalid_characters(str_json, xml_tmp_file):
    str_xml = (
        "<data>" + str_json.replace("&", "&#38;").replace("<", "&#60;") + "</data>"
    )
...
async def transform_to_rdf(str_submitted_json, output_format="xml"):
    if not validate_json(str_submitted_json):
        raise HTTPException(
            status_code=500, detail=f"Error, msg: The given json input is not valid."
        )
