...
<xsl:stylesheet xmlns:dc="http://purl.org/dc/elements/1.1/"
    ...
    <xsl:template match="/map" xpath-default-namespace="http://www.w3.org/2005/xpath-functions">
        ...
        <xsl:for-each select="./map[@key='metadata']/map[@key='subject_keywords_aat']/array[@key='value']/map">
            <xsl:text>ohs.metadata.subject_keywords_aat.</xsl:text>
            <xsl:value-of select="string[@key='label']" />
            <xsl:text>=</xsl:text>
            <xsl:value-of select="string[@key='value']" />
            <xsl:text>&#10;</xsl:text>
        </xsl:for-each>
        <xsl:for-each select="./map[@key='metadata']/map[@key='subject_keywords_wikidata']/array[@key='value']/map">
            <xsl:text>ohs.metadata.subject_keywords_wikidata.</xsl:text>
            <xsl:value-of select="string[@key='label']" />
            <xsl:text>=</xsl:text>
            <xsl:value-of select="string[@key='value']" />
            <xsl:text>&#10;</xsl:text>
        </xsl:for-each>
        <xsl:for-each select="./map[@key='metadata']/map[@key='subject_keywords_freetext']/array[@key='value']/map">
            <xsl:text>ohs.metadata.subject_keywords_freetext.</xsl:text>
            <xsl:value-of select="string[@key='label']" />
            <xsl:text>=</xsl:text>
            <xsl:value-of select="string[@key='value']" />
            <xsl:text>&#10;</xsl:text>
